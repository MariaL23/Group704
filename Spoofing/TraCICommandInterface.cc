...
void TraCICommandInterface::Vehicle::changeLane(uint8_t laneIndex, double duration) {
    uint8_t commandId = 0x13;  // Command ID for changeLane
    uint8_t variableType = TYPE_COMPOUND;  // Compound object type
    int32_t compoundSize = 2;  // Number of elements in the compound object
    ...
    TraCIBuffer buf = connection->query(
        ...
        TraCIBuffer()
            ...
            << nodeId
            << variableType
            << compoundSize  // Define compound size
            << TYPE_BYTE << laneIndex  // Lane index as a byte
            << TYPE_DOUBLE << duration  // Duration as a double
    );
